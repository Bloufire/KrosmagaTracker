...
namespace AddOn_Krosmaga___Blou_fire
{
	public class UIPlayerDatas : INotifyPropertyChanged
	{
		...
		{
			get { return _deck.OrderByDescending(x => x.Card.GodType).ThenBy(x => x.Card.CostAP).ToList(); }
		...
		{
			get { return _deck.Where(x => x.Card.Rarity == 4).ToList(); }
		...
		{
			get { return _deck.Where(x => x.Card.Rarity == 3).ToList(); }
		...
		{
			get { return _matchsWithFilters.OrderByDescending(x => x.Date).ToList(); }
		...
		{
			get { return _cardAlreadyPlayed; }

			set { _cardAlreadyPlayed = value; }
		}

		public ObservableCollection<DeckUI> CardsInHandUI
		{
			get { return new ObservableCollection<DeckUI>(CardsInHand); }
		...
		{
			get { return new ObservableCollection<DeckUI>(DeckInfinites); }
		...
		{
			get { return new ObservableCollection<DeckUI>(DeckKrosmiques); }
		...
		private void UpdateMatchsWithFilterList()
		{
			...
			MatchsWithFilters = MatchsList.Where(x =>
					...
					(OpponentClasseFilter == "Tous" || OpponentClasseFilter == null
						? true
						: x.Deck.OpponentClasse == OpponentClasseFilter) &&
					(String.IsNullOrEmpty(OpponentNameFilter)
						? true
						: x.OpponentName.ToLower().Contains(OpponentNameFilter.ToLower())))
				.ToList();
			NotifyPropertyChanged("MatchsWithFilters");
		}
